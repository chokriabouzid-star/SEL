...
impl fmt::Display for SovereignError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NonDeterministicNumber =>
                write!(f, "[SEL-503] Non-deterministic number: floating point or big integer"),
            Self::InternalError(msg) =>
                write!(f, "[SEL-901] Internal error: {}", msg),
        }
    }
}
...
impl From<std::io::Error> for SovereignError {
    fn from(err: std::io::Error) -> Self {
        SovereignError::WorkspaceCreationFailed(
