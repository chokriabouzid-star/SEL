...
use crate::{SelResult, SovereignError};
...
pub fn canonicalize_json(json_str: &str) -> SelResult<String> {
    let value: Value = serde_json::from_str(json_str)
        .map_err(|e| SovereignError::InvalidMissionFormat(format!("Invalid JSON: {}", e)))?;

    let canonical = canonicalize_json_value(&value)?;

    serde_json::to_string(&canonical)
        .map_err(|e| SovereignError::InvalidMissionFormat(format!("Failed to serialize: {}", e)))
}
...
pub fn canonicalize_json_value(value: &Value) -> SelResult<Value> {
    match value {
        ...
        Value::Number(n) if n.is_f64() => Err(SovereignError::InvalidMissionFormat(
