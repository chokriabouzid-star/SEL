...
pub fn validate_security_rules(actions: &[ValidatedAction]) -> Option<SecurityViolation> {
    for action in actions {
        ...
        if action.command.contains(';') || action.command.contains('|') || 
           action.command.contains('&') || action.command.contains('$') || 
...
mod tests {
    ...
    fn test_dangerous_pattern_in_command() {
        ...
        let result = validate_security_rules(&[action]);
        assert!(matches!(result, Some(SecurityViolation::DangerousPattern(_))));
    }
